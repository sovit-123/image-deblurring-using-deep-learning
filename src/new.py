...
mot_blur = os.listdir('../input/motion_blurred/')
mot_blur.sort()
...
x_blur = []
for i in range(len(mot_blur)):
    x_blur.append(mot_blur[i])
...
class DeblurDataset(Dataset):
    ...
    def __getitem__(self, i):
        blur_image = plt.imread(f"../input/motion_blurred/{self.X[i]}")
        
        if self.transforms:
            blur_image = self.transforms(blur_image)
            
        if self.y is not None:
            sharp_image = plt.imread(f"../input/sharp/{self.y[i]}")
...
class Autoencoder(nn.Module):
    def __init__(self):
        ...
        # encoder layers
        self.enc1 = nn.Conv2d(1, 128, kernel_size=5, padding=1)
        ...
        # self.enc5 = nn.Conv2d(64, 32, kernel_size=1)
        ...
        self.out = nn.ConvTranspose2d(128, 1, kernel_size=1)
...
def fit(model, dataloader, epoch):
    ...
    for i, data in tqdm(enumerate(dataloader), total=int(len(train_data)/dataloader.batch_size)):
        ...
        optimizer.zero_grad()
        outputs1 = model(blur_image)
        outputs2 = model(sharp_image)
...
def validate(model, dataloader, epoch):
    ...
    with torch.no_grad():
        ...
        # if i == 1:
        #     save_decoded_image(sharp_image.cpu().data, name=f"../outputs/saved_images/sharp{epoch}.jpg")
        save_decoded_image(outputs1.cpu().data, name=f"../outputs/saved_images/val_deblurred{epoch}.jpg")
